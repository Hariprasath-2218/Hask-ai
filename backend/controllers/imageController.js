...
const generateImageToImage = async (req, res) => {
  try {
    ...
    try {
      ...
      // Construct the prompt for Gemini to describe the desired modification
...
Make sure the output is a single, coherent, highly descriptive text prompt suitable for a modern text-to-image AI.`;
      ...
      const hfResponse = await fetch(
        "https://router.huggingface.co/nebius/v1/images/generations",
        {
          ...
          body: JSON.stringify({
            response_format: "b64_json",
            // Use the detailed description from Gemini as the prompt for Hugging Face
      ...
      const image = new Image({
        userId,
        prompt: prompt.trim(), // Store the original user prompt
        imageUrl,
        isImageToImage: true,
        originalImageName: imageFile.originalname,
        geminiDescription: modificationDescription // Optionally store Gemini's full description
      });
